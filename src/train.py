...
def test_model_2(model, test_dataset, test_loader):
    model.eval()
    labels = np.zeros(len(test_dataset))
    predictions = np.zeros(len(test_dataset))
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_loader):
            ...
            labels[start:end] = labels_batch.argmax(dim=-1).cpu().numpy()
            predictions[start:end] = predictions_batch.cpu().numpy()
    return labels, predictions
...
if __name__ == "__main__":
    ...
    for i, (model, optimizer) in enumerate(zip(models, optimizers)):
        ...
        if (problem_num == 1):
            labels, predictions = test_model_2(model, test_dataset, test_loader)
        else:
