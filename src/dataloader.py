...
class CrisisDataset(Dataset):
    def __init__(self, data_path, tokenizer, max_length):
        self.data = pd.read_csv(data_path, encoding='utf-8', sep="\t", on_bad_lines='skip', header=0)
        self.data = self.data[self.data['label'].notna()]
...
class StockDataset(Dataset):
    def __init__(self, data_path, tokenizer, max_length):
        ...
        self.data['Concatenated_Top'] = self.data.apply(lambda row: ';'.join(row[2:8]), axis=1)
        self.data = self.data[self.data['Label'].notna()]
        self.data = self.data[pd.to_numeric(self.data['Label'], errors='coerce').notnull()]
        self.tokenizer = tokenizer
        self.max_length = max_length
